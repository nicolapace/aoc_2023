...
fn main() {
    ...
    /* COUNT ALL BLOCKS THAT WILL FALL */
    let mut num_will_fall = 0;
    for block in &up_n_down{ 

        if block.1.1.len() > 0 {
            let mut will_fall : HashSet<u32> = HashSet::new();
            let mut fallen : HashSet<u32> = HashSet::new();

            for up_block in &block.1.1{
                if up_n_down.get(&up_block).unwrap().0.len() == 1 {
                    fallen.insert(up_block.clone());
                    will_fall.insert(up_block.clone());
                }
            }

            loop {
                
                let mut new_fallen : HashSet<u32> = HashSet::new();
                for b in &fallen{
                    let blocks_over_b = up_n_down.get(&b).unwrap().1.clone();

                    //now I will check all the blocks_over_b, if all their bases are in will_fall, they will fall
                    //use will_fill because there can be blocks that extend over the z axis
                    for block_over_b in blocks_over_b {
                        let mut all_bases_fallen = true;
                        let bases_of_block_over_b = up_n_down.get(&block_over_b).unwrap().0.clone();
                        for base_of_block_over_b in bases_of_block_over_b {
                            if will_fall.get(&base_of_block_over_b) == None{
                                all_bases_fallen = false;
                            }
                        }
                        if all_bases_fallen{
                            new_fallen.insert(block_over_b);
                        }
                    }
                }
                if new_fallen.len() == 0 {
                ...
                fallen = new_fallen.clone();
                will_fall.extend(new_fallen);
            }
            num_will_fall+=will_fall.len();

            //println!("block: {}\t- num: {}",block.0,will_fall.len());
        }
    }
    println!("result: {}", num_will_fall);
}