...
fn main() {
    ...
    for seq in sequences {
        ...
        while !all_zeros{
            ...
            for i in 0..=seq.len()-row{
                new_row[i] = matrix[matrix.len()-1][i+1]-matrix[matrix.len()-1][i];
                if all_zeros && new_row[i]!=0 {
                    all_zeros=false;
                }
            }
            row+=1;
        ...
        for i in 2..=matrix.len() {
            backward_history = matrix[matrix.len()-i][0]-backward_history;
        }
        print!("{:?} - ", matrix);
        println!("backward_history {:?}", backward_history);
        sum+=backward_history;
    }
    println!("result: {:?}", sum);
}
