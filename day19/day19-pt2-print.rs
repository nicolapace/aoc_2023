...
fn main() {
    ...
    let mut workflows_graph : Vec<(Vec<&str>, Vec<String>, ((u64,u64),(u64,u64),(u64,u64),(u64,u64)))> = vec![(vec!["in"],vec![],((1,4000),(1,4000),(1,4000),(1,4000)))];

    let mut all_ended = false;
    while !all_ended {
        all_ended = true;
        let mut new_workflows_graph : Vec<(Vec<&str>, Vec<String>, ((u64,u64),(u64,u64),(u64,u64),(u64,u64)) )> = vec![];
                ...
                for i in 0..steps.len() {
                    ...
                    for j in 0..i {
                        ...
                        if instruction.len()!=1 {
                            new_workflow.1.push( "!".to_owned()+instruction[0]);

                            let minor = instruction[0].split("<").collect::<Vec<_>>();
                            if minor.len()>1 {
                                let num = minor[1].parse::<u64>().unwrap();
                                if minor[0] == "x" {
                                    if num > new_workflow.2.0.0 {
                                        new_workflow.2.0.0 = num;
                                    }
                                } else if minor[0]=="m" {
                                    if num > new_workflow.2.1.0 {
                                        new_workflow.2.1.0 = num;
                                    }                                
                                } else if minor[0]=="a" {
                                    if num > new_workflow.2.2.0 {
                                        new_workflow.2.2.0 = num;
                                    }                                                            
                                }else if minor[0]=="s" {
                                    if num > new_workflow.2.3.0 {
                                        new_workflow.2.3.0 = num;
                                    }                                                          
                                }
                            }else {
                                ...
                                if major[0] == "x" {
                                    if num < new_workflow.2.0.1 {
                                        new_workflow.2.0.1 = num;
                                    }  
                                } else if major[0]=="m" {
                                    if num < new_workflow.2.1.1 {
                                        new_workflow.2.1.1 = num;
                                    }
                                } else if major[0]=="a" {
                                    if num < new_workflow.2.2.1 {
                                        new_workflow.2.2.1 = num;
                                    }
                                }else if major[0]=="s" {
                                    if num < new_workflow.2.3.1 {
                                        new_workflow.2.3.1 = num;
                                    }
                                }
                            }
                        }
                    ...
                    if instruction.len()==1 {
                        ...
                        new_workflow.1.push(instruction[0].to_string());
    ...
    for el in workflows_graph {
        println!("\n{:?}",el);
        sum += (1+el.2.0.1-el.2.0.0)*(1+el.2.1.1-el.2.1.0)*(1+el.2.2.1-el.2.2.0)*(1+el.2.3.1-el.2.3.0);
    }
